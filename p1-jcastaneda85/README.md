...
| Name | Email Address |
|------|---------------|
| Hubert Tran | Huberttran@csu.fullerton.edu |
| Dominic Wrtaza | dwrtaza2@csu.fullerton.edu |
| Joshua Castaneda | jcastaneda85@csu.fullerton.edu |
| Dylan Dao | dylandao104@csu.fullerton.edu |
| Trent Nguyen | trentnguyen21@csu.fullerton.edu |

## Programming Language

**Python 3** - This project is implemented using Python 3.6 or higher.
...
3. **Use FTP commands**: Enter commands like `LS`, `GET <filename>`, `PUT <filename>`, or `QUIT`

## Special Notes

- The server must be started before the client connects
- Server and client communicate over localhost (127.0.0.1) on port 5002
- The `server_files/` directory contains files available for download
- The `client_files/` directory is where downloaded files are stored
- All file transfers use a custom JSON-based protocol over TCP sockets
- The implementation follows a request-response pattern for all operations
